...
}) 
{
    ...
    const {Notes,editIconColor}=notesState
    const ViewAllDoc=state.viewAllImages
    const {displaySendToDiv}=CommonFunctions()
    const {FilesData,deleteNotes,sendFileFunction,showAdvanceFeatures}=NotesFunc()
...
return (
    ...
    {Notes!==undefined && Notes.length ? Notes.map((ImageName,index) =>{  
    ...
    <>
        ...
        <form key={index} id="textDoc_form" style={Notes_ViewAllDoc_styles} > 
           <div id={ViewAllDoc?"select_div_box_notes2" : "select_div_box_notes"} className={`select_div_box${index}`}  style={{display:selectMultiple?'block':'none',borderRadius:"8px"}}>
            <input 
        ...
        <div id='textDoc_message_icons_and_heading'>
            ...
            <AiFillDelete id='textDoc_delete' size={29} onClick={()=>deleteNotes(ImageName.heading,ImageName.body)} />  
        ...
        <div id="textDoc_Image_Card" style={{width:ViewAllDoc?'93%':'89%',height:ViewAllDoc?'89%':'100%',left:ViewAllDoc?'3.5%':'0%',backgroundColor:ViewAllDoc?'black':null}}>
            ...
            <div id="notes_heading" style={notes_heading_ViewAllDoc_styles}>
                ...
                {!selectMultiple?ImageName.heading:null}
